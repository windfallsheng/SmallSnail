 ...
 * @date 2018年11月06日
 * 主要职责:增强数据操作功能的内聚性,
 * 假如登录操作包含了若干个网络请求去完成，就可以在本类下的{@link MainActivityModel#login(String, String, RequestCallBack)}
 * 方法里完成，将结果返回给上一次。
 * todo 可以结合切面编程的思想，实现对网络请求的返回结果进行逻辑处理，包括各类异常的不同处理，及返回数据不同code码的处理逻辑
 */
public class MainActivityModel implements MainActivityContract.IMainActivityModel {
