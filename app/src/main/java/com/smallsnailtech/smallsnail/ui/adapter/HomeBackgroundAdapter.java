 ...
 * @date 2018年11月06日
 */
public class HomeBackgroundAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public HomeBackgroundAdapter(Context context, List<String> data) {
        mContext = context;
        mData = data;
    }
...
//        }
        ...
        switch (viewType) {
            case DATA_TYPE_SPLASH_IMAGE:
                return new ViewHolderImage(
                        LayoutInflater.from(mContext).inflate(R.layout.view_holder_background_type_image, parent, false));
            case DATA_TYPE_SPLASH_MEDIA:
                return new ViewHolderMedia(
                        LayoutInflater.from(mContext).inflate(R.layout.view_holder_background_type_media, parent, false));
            default:
                return new ViewHolderImage(
                        LayoutInflater.from(mContext).inflate(R.layout.view_holder_background_type_image, parent, false));
        }
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof ViewHolderImage) {
            ...
            Glide.with(mContext).load(url).into(((ViewHolderImage) holder).ivImage);
